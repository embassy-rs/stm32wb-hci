...
async fn le_set_advertising_enable() {
    let sink = RecordingSink::new();
    let _ = sink.le_set_advertising_enable(true).await;
...
async fn le_set_scan_enable() {
    let sink = RecordingSink::new();
    let _ = sink.le_set_scan_enable(true, false).await;
...
async fn le_add_anon_advertising_devices_to_white_list() {
    let sink = RecordingSink::new();
    let _ = sink.le_add_anon_advertising_devices_to_white_list().await;
    assert_eq!(
        sink.written_data(),
